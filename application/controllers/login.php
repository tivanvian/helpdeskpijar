...
function index()
  ...
  function login_akses()
  {
  	...
  	$nrk = trim($this->input->post('nrk'));
  	$password = md5(trim($this->input->post('password')));

	$akses = $this->db->query("select A.username,B.nama, B.nrk, A.level, B.id_jabatan, C.id_dept FROM user A
		LEFT JOIN karyawan B ON B.nik = A.username
        LEFT JOIN bagian_departemen C ON C.id_bagian_dept = B.id_bagian_dept
	 WHERE B.nrk = '$nrk' AND A.password = '$password'");
